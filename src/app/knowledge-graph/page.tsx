        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 mb-2 items-start">
          ...
          <div className="lg:col-span-8 flex flex-col gap-6">
            ...
            <div className="flex justify-end">
              <button onClick={handleProcessText} disabled={isProcessing} className="flex items-center gap-3 px-10 py-5 bg-slate-900 text-white rounded-2xl font-black uppercase tracking-widest text-[12px] hover:bg-indigo-600 transition-all shadow-xl active:scale-95 disabled:bg-slate-300">
                {isProcessing ? <><RefreshCcw className="w-4 h-4 animate-spin" /> Extracting...</> : <>Map Relationships</>}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
          ...
          <div className="space-y-6">
            {selectedNode ? (
              ...
              <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden h-full min-h-[400px] flex flex-col justify-center text-center">
                <div className="absolute -top-10 -right-10 w-40 h-40 bg-indigo-500/10 rounded-full blur-3xl"></div>
                <h3 className="text-2xl font-black uppercase tracking-tight mb-4 leading-tight">Relational Logic</h3>
                <p className="text-sm text-slate-400 font-bold leading-relaxed">Click any character node to explore standing.</p>
              </div>
            )}
          </div>
        </div>
