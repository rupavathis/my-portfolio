...
// Force Graph needs to be dynamic for SSR compatibility
...
const MERCHANT_DATA = {
  nodes: [
    { id: 'Antonio', type: 'Merchant' },
    { id: 'Bassanio', type: 'Friend' },
    { id: 'Portia', type: 'Heiress' },
    { id: 'Shylock', type: 'Moneylender' },
    { id: 'Jessica', type: 'Daughter' },
    { id: 'Lorenzo', type: 'Friend' },
  ],
  links: [
    { source: 'Antonio', target: 'Bassanio', label: 'friend / guarantor' },
    { source: 'Bassanio', target: 'Portia', label: 'suitor / husband' },
    { source: 'Antonio', target: 'Shylock', label: 'owes money / rival' },
    { source: 'Shylock', target: 'Jessica', label: 'father' },
    { source: 'Jessica', target: 'Lorenzo', label: 'elopes' },
    { source: 'Lorenzo', target: 'Bassanio', label: 'friend' },
    { source: 'Portia', target: 'Antonio', label: 'defends / saves' },
  ]
};
...
export default function KnowledgeGraphPage() {
  ...
  useEffect(() => {
    if (graphRef.current && hasProcessed) {
      ...
      setTimeout(() => {
        if (graphRef.current) graphRef.current.d3AlphaTarget(0);
      }, 10000);
    }
  }, [hasProcessed]);
    ...
    <div className="min-h-screen bg-[#FDFEFF] text-slate-900 font-sans p-6 md:p-10">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-200 p-10 max-w-md w-full relative overflow-hidden text-center">
              ...
              <div className="relative z-10">
                ...
                <p className="text-sm text-slate-500 font-bold leading-relaxed mb-8">The complete relational web has been saved and is ready to be shared with your research team.</p>
                <div className="w-full space-y-3 mb-8">
                   <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left ml-1">Suggested Outlets</div>
      ...
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 mb-10">
          <header className="lg:col-span-4 space-y-6">
            ...
            <p className="text-xs text-indigo-700 font-black tracking-[0.3em] uppercase">Relational Discovery Tool</p>
          </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
          <div className="lg:col-span-3 bg-white rounded-[3rem] border-4 border-slate-100 shadow-2xl overflow-hidden h-[600px] relative flex items-center justify-center" ref={containerRef}>
            {!hasProcessed ? (
              <div className="text-center space-y-6 animate-in fade-in duration-700"><div className="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mx-auto border-2 border-dashed border-slate-200"><Network className="w-8 h-8 text-slate-300" /></div><div className="space-y-2"><h3 className="text-xl font-black text-slate-400 uppercase tracking-tight">Graph Pending</h3><p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Awaiting Entity Extraction...</p></div></div>
            ) : (
          ...
          <div className="space-y-6">
            {selectedNode ? (
              ...
              <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden h-full min-h-[400px]"><div className="absolute -top-10 -right-10 w-40 h-40 bg-indigo-500/10 rounded-full blur-3xl"></div><BookOpen className="w-10 h-10 text-amber-400 mb-8" /><h3 className="text-2xl font-black uppercase tracking-tight mb-4 leading-tight">Relational Logic</h3><p className="text-sm text-slate-400 font-bold leading-relaxed mb-8">Click on any character to explore their social Standing.</p><div className="flex items-center gap-3 p-4 bg-white/5 rounded-2xl border border-white/10"><Search className="w-4 h-4 text-indigo-400" /><span className="text-[10px] font-black uppercase tracking-widest text-indigo-300">Ready</span></div></div>
            )}
          </div>
        </div>
      </div>
